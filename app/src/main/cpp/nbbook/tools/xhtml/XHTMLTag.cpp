...
XHTMLTag::XHTMLTag(const std::string &tag, const std::vector<std::string> &classes) : Tag(tag),
                                                                                      Classes(classes) {
}

bool XHTMLTag::matches(const CSSSelector &selector) const {
    if (selector.Tag == "*") {
...
int XHTMLTagList::find(const CSSSelector &selector, int from, int to) const {
    if (from < 0) {
...
bool XHTMLTagList::matches(const CSSSelector &selector, int index) const {
