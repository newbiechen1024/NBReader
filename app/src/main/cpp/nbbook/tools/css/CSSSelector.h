...
public:
    ...
    struct Component {
        Component(Relation delimiter, std::shared_ptr<CSSSelector> selector);

        const Relation Delimiter;
        const std::shared_ptr<CSSSelector> Selector;
    };

public:
    static std::shared_ptr<CSSSelector> parse(const std::string &data);

private:
    static void update(std::shared_ptr<CSSSelector> &selector, const char *&start, const char *end,
                       char delimiter);
