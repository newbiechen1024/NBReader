...
class EncryptionMap {
public:
    void addInfo(const FileDir &dir, std::shared_ptr<FileEncryptionInfo> info);

    std::shared_ptr<FileEncryptionInfo> info(const std::string &path) const;

private:
    std::map<std::string, std::shared_ptr<FileEncryptionInfo> > myPathToInfo;
};
