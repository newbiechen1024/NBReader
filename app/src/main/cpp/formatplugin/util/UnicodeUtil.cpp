...
bool UnicodeUtil::isUtf8String(const char *str, int len) {
...
bool UnicodeUtil::isUtf8String(const std::string &str) {
    return isUtf8String(str.data(), str.length());
}

void UnicodeUtil::cleanUtf8String(std::string &str) {
    int charLength = 0;
    int processed = 0;
...
int UnicodeUtil::utf8Length(const char *str, int len) {
    const char *last = str + len;
    int counter = 0;
    while (str < last) {
        if ((*str & 0x80) == 0) {
...
int UnicodeUtil::utf8Length(const std::string &str) {
    return utf8Length(str.data(), str.length());
}

int UnicodeUtil::length(const char *str, int utf8Length) {
    const char *ptr = str;
...
int UnicodeUtil::length(const std::string &str, int utf8Length) {
...
std::string UnicodeUtil::toLower(const std::string &utf8String) {
    if (utf8String.empty()) {
        return utf8String;
    }
    ...
    // TODO:c++ 无法处理 UTF-8 所以借用 Java 的方法去处理???
    
    JNIEnv *env = AndroidUtil::getEnv();
    jstring javaString = AndroidUtil::toJString(env, utf8String);
    jstring lowerCased = AndroidUtil::Method_String_toLowerCase->callForJavaString(javaString);
    if (javaString == lowerCased) {
        env->DeleteLocalRef(lowerCased);
        env->DeleteLocalRef(javaString);
        return utf8String;
    } else {
        const std::string result = AndroidUtil::toCString(env, lowerCased);
        env->DeleteLocalRef(lowerCased);
        env->DeleteLocalRef(javaString);
        return result;
    }
...
std::string UnicodeUtil::toUpper(const std::string &utf8String) {
    if (utf8String.empty()) {
        return utf8String;
    }
    ...
    JNIEnv *env = AndroidUtil::getEnv();
    jstring javaString = AndroidUtil::toJString(env, utf8String);
    jstring upperCased = AndroidUtil::Method_String_toUpperCase->callForJavaString(javaString);
    if (javaString == upperCased) {
        env->DeleteLocalRef(upperCased);
        env->DeleteLocalRef(javaString);
        return utf8String;
    } else {
        const std::string result = AndroidUtil::toCString(env, upperCased);
        env->DeleteLocalRef(upperCased);
        env->DeleteLocalRef(javaString);
        return result;
    }
