...
static struct LineBreakProperties *get_lb_prop_lang(const char *lang) {
    ...
    if (lang != NULL) {
        /**
         * lb_prop_lang_map 是作者定义的 LineBreakPropertiesLang 对象数组
         * LineBreakPropertiesLang 数组在全局中存储了各国的  LineBreakPropertiesLang
         * LineBreakPropertiesLang 中存储了各国的 LineBreakProperties 参数
         */
        for (lbplIter = lb_prop_lang_map; lbplIter->lang != NULL; ++lbplIter) {
            /**
             * 根据当前的 lang 获取对应的 LineBreakProperties
             */
...
void set_linebreaks(
    ...
    /*将 s 字符串中的字符通过 get_next_char 函数转换成 utf-8 或 utf-16 或 utf-32 的字符，取决于 get_next_char 传入的函数。*/
    /*get_next_char 传入的函数不分析，需要了解 java 字符集的知识。*/
    ch = get_next_char(s, len, &posCur);
    if (ch == EOS)
        return;

    /*根据语言获取 LineBreakProperties 对象*/
...
void set_linebreaks_utf16(
        ...
        const char *lang,
        char *brks) {
    set_linebreaks(s, len, lang, brks, (get_next_char_t) lb_get_next_char_utf16);
}
